...
#Obs v pred
fit_pred = posterior_epred(mod_wst, filter(wst_dat, !is.na(nor_wsthd) & !is.na(smolt_adj)))

obs_pred = tibble(year = filter(wst_dat, !is.na(nor_wsthd) & !is.na(smolt_adj))$year,
  ...
  left_join(wst_dat %>% dplyr::select(year, nor_wsthd)) %>%
  mutate(q.025 = ifelse(q.025 < 0, 0, q.025)) %>%
  pivot_longer(cols = c('med','nor_wsthd')) %>%
  ggplot()+
